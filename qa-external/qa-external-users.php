...
*/
	{
		...
		define('ROOT_PATH', "/data/ubuntuvn/forum.ubuntu-vn.org/public_html");
...
*/
	...
	*/
		...
		$qa_db_connection=qa_db_connection();
		$result = mysql_query("SELECT user_email FROM phpbb_users WHERE user_id='".mysql_real_escape_string($userid)."'");
		while ($row = mysql_fetch_assoc($result)){
			if (is_array($row))
...
*/
	...
	*/
		...
		if (count($publicusernames)) {
			...
			foreach ($publicusernames as $publicusername)
				$escapedusernames[]="'".mysql_real_escape_string($publicusername)."'";
			
			$results=mysql_query(
				'SELECT username, user_id FROM phpbb_users WHERE username IN ('.implode(',', $escapedusernames).')'
			);
	
			while ($row = mysql_fetch_assoc($results))
				$publictouserid[$row['username']]=$row['user_id'];
		}
...
*/
	...
	*/
		...
		if (count($userids)) {
			...
			foreach ($userids as $userid)
				$escapeduserids[]="'".mysql_real_escape_string($userid)."'";
			
			$results=mysql_query(
				'SELECT username, user_id FROM phpbb_users WHERE user_id IN ('.implode(',', $escapeduserids).')'
			);
	
			while ($row = mysql_fetch_assoc($results))
				$useridtopublic[$row['user_id']]=$row['username'];
		}
...
*/
	...
	//	By default, show the public username linked to the Q2A profile page for the user
		...
		$publicuserid=$logged_in_user['userid'];
		$publicusername=$logged_in_user['publicusername'];
		
		return '<a href="'.htmlspecialchars('http://forum.ubuntu-vn.org/memberlist.php?mode=viewprofile&u='.urlencode($publicuserid)).
	...
	//	By default, show the public username linked to the Q2A profile page for each user
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]='<a href="'.htmlspecialchars('http://forum.ubuntu-vn.org/memberlist.php?mode=viewprofile&u='.urlencode($userid)).
...
*/
	...
	*/
		...
		$phpEx = "php";
		$results=mysql_query(
			'SELECT user_avatar FROM phpbb_users WHERE user_id = ' . $userid
		);
		while ($row = mysql_fetch_assoc($results)) {
			$avatar = $row['user_avatar'];
		}
		...
		return '<img src="http://forum.ubuntu-vn.org/download/file.php?avatar='.htmlspecialchars($avatar) . '" ' .
